...
$(G,\sigma)$ is
  ...
  is consistent in order to verify that $(G,\sigma)$ is a .  In this case, every tree $(T,\sigma)$ with leaf set
  $V(G)$ that agrees with
...
\mh{
%In this section, we always consider 3-colored digraphs $(G,\sigma)$. 
...
%\OLD{
%In the general case, \emph{s-color derivative} denotes $\partial_s(G)$ for a color $s$, and \emph{color derivatives} denotes $\partial_s(G)$ for any color $s$. An example of the color derivatives of $G$ is in Figure~\ref{fig:color_derivatives}. When $\partial_s(G)$ is a 2-BMG, $T_s$ denotes a tree explaining it.
%}
%In general, color derivatives of $G$ are not 2-BMGs even though $G$ is a 3-BMG, as \mh{illustrated in Fig.\ \ref{fig:color_derivatives}.}
...
%  \centering
%    \includegraphics[width=16cm]{figures/different_trees.png}
%    \caption{Red- and blue-derivative of $G$ which is not 3-BMG. The tree $T_r$ explains $\partial_r(G)$ while the tree $T_b$ explains $\partial_b(G)$.
%    	In Figure~\ref{fig:different_trees}, the uncolored trees associated to
%    	$T_r$ and $T_b$ are isomorphic but do not coincide. Let $t$ be the
%    	parent of $y'$ in $T_r$, and let $t'$ be the parent of $y$ in $T_b$. The
%    	bijection $\phi:V(T_r)\rightarrow V(T_b)$ defined by
...
%Let $(G,\sigma)$ be an $n$-colored digraph where $\sigma\colon V(G) \to S$, 
%$S = \{1,\dots,n\}$ and $i,j\in S$ are distinct. Moreover, let $(G_i,\sigma_i)$
%be the $i$-color derivative of $(G,\sigma)$.
%Then $(\Delta_{ij}(\Sigma,\sigma)$ is defined to be the 2-colored digraph $(G_i^j,\sigma_i^j)$
%such that 
%\begin{enumerate}
%    \item $G$ and $G_i^j$ have the same vertex set, i.e.,  $V(G_i^j) \coloneqq V(G)$.
%    \item $\sigma_i\colon V(G_i) \to \{i,c\}$ where $c\notin S$
%    	  and $\sigma_i(x)=c$ if and only if $\sigma(x)\neq i$. 
%    	  
%    	  In other words,  $G$ and $G_i$ have the same vertices with color $i$ 
%    	  whereas any other vertex of $G_i$ has color $c$.  
%    
%    \item An edge $xy\in E(G)$ is an edge of $G_i$ presicely if 
%    	  $\sigma(x)=i	$ or  $\sigma(y)=i$.
%    	  
%    	  In other words, the edges of $G_i$ are precisely those edges 
%    	  of $G$ that are incident to a vertex with color $i$.
...
%on the vertex set of $G$ such that
...
\TODO{should be adjusted to new def, but possibly becomes trivial, } % since former $\Delta_{rb}(\partial_r(G))$ is (G_)}
\begin{figure}[ht]
  \centering
\begin{tikzcd}
(\partial_b(G),\sigma_b) 
& [3em](G,\sigma) \arrow{l}[sloped,above]{\text{}} \arrow[r]
& [3em](\partial_r(G),\sigma_r) \arrow[d,blue] \arrow[ll,{bend left=30},"\Delta_{(r,b)}",red]\\
