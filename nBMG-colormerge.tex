...
\ak{
	...
	\TODO{MH: no, it only shows that this "each-class-singleton" partition is needed, under the assumption that "there is an informative triple for every two distinct colors of $S$." .. 
	it would be nice to understand, if this holds in general for all BMGs, or if there are "more interesting" partitions for BMGs ie BMGs for which not every two colors provide some triple}
...
\begin{proof}
   ...
   \ref{prop:BMG-triple-charac} implies that $(G_\Pi,\sigma_\Pi)$ is a BMG. 
    ...
    ,\mathscr{F}(G_\Pi,\sigma_\Pi) )$ is consistent \ak{Why?}
