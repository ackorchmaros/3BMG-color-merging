...
\begin{proof}
 ...
 $\sigma_\Pi(z)=S_i$. To show that $(G_\Pi,\sigma_\Pi)$ is color-sink-free we
	must show $N_{G_\Pi}(x,S_i)\neq \emptyset$ \OLD{for all $S_i\in \Pi\setminus
	\{\sigma(x)\}$.} \ak{for all $S_i$ with $\sigma(x)\notin S_i$ and for each $x\in V(G)$, for example, if $\Pi=\{\{1\}, \{2,3\}\}=S_1 \cup S_2$ and $\sigma(x)=2$ we check only $N_{G_\Pi}(x,S_1)\neq \emptyset$}. \OLD{The latter is equivalent to show that $N_{G_\Pi}(x,S_i)\neq
	\emptyset$ for all $S_i\in \Pi$ with $x\notin S_i$}. Let $x\in V(G_\Pi)=V(G)$
	and suppose that $\ak{\sigma(}x\ak{)}\in S_j\in \Pi$ and thus, $\sigma_\Pi(x) = S_j$. Now,
...
\begin{proof}
	...
	~\ref{lem:R-Ri}, (R1) and (F1) are satisfied. Moreover, by Proposition~\
	...
	,\mathscr{F}(G,\sigma))$ is consistent. Now, we can apply Proposition~\
	...
	\mathscr{F}(G_\Pi,\sigma_\Pi))$ is consistent, we can apply Proposition~\
	\ref{prop:BMG-triple-charac} to conclude that $(G_\Pi,\sigma_\Pi)$ is a BMG.
\end{proof}

\ak{
	The following observation suggests that the only partition of $S$ satisfying (R1) for $(G,\sigma)$ is the partition of $|S|$ color classes each consisting of a single color.
	\begin{observ}
	Let $(G,\sigma)$ be a properly colored graph whose induced subgraphs on any two colors are not complete, and there are at least two distinct vertices for every color of $S$. Then $(G,\sigma)$ is color-sink-free if and only if there is an informative triple for every two distinct colors of $S$.
