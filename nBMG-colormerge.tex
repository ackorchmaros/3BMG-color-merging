...
\label{def:color-derivative}
	Let $(G,\sigma)$ be an $n$-colored digraph and $S'\subseteq \in S$. We
...
%
	...
	$2$-colored digraph $(G_{|S'},\sigma^2_{|S'})$. \smallskip
	...
	Note, for $S' = \{i\}$, the colorings $(G_{|S'},\sigma^2_{|S'})$
	and $(G_{|i},\sigma_{|i})$ are identical up to recoloring vertices with color $c'$
	in $(G_{|S'},\sigma^2_{|S'})$ by color $i$ and we also write 
	 $(G_{|i},\sigma^2_{|i})$ instead.
...
\TODO{MH:what are necassary/sufficient conditions on collections $\mathcal{S}$ of subsets $S'\subseteq S$
